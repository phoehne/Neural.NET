...
namespace Neural.Data {
	...
	public class FileDataReader : DataReader {
		...
		private void OpenFile() {
			...
			if(!theFile.Exists) {
				throw new FileNotFoundException("Unable to find data reader input file.");
			}

			inputFile = theFile.OpenText();
		}
		...
		private string[] ReadNextLine() {
			...
			if(newLine.Trim().Length > 0) {
				result = newLine.Split(delimiter.ToCharArray());
				if(result.Length != fields.Count) {
					throw new DataReadingException("Line count does not match field count.");
				}
			}
			return result;
		}
		...
		public override List<Dictionary<String, double>> ReadValues() {
            List<Dictionary<String, double>> result = new List<Dictionary<String, double>>();
			try {
				string[] aLine = ReadNextLine();
				while(aLine != null) {
					double[] transLine = TranslateLine(aLine);
					double[] scaledLine = ScaleLine(transLine);
					result.Add(NameValues(scaledLine));
					aLine = ReadNextLine();
				}
			} catch(FileNotFoundException fnfe) {
				throw new DataReadingException("Failed to read data due to file exception", fnfe);
			}
			return result;
		}
	}
